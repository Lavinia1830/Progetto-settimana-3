...
function splitMe(str){
    let splitString = str.split(" "); //quando ci metto lo spazio mi seleziona le parole. Quando non metto lo spazio mi seleziona ogni singola lettera
...
function isThisAnEmail(str){
    if(str === "lavi.baratti@gmail.com"){
        console.log(true);
    } else {
        console.log(false);
    }
}
isThisAnEmail("lavi.baratti@gmail.com");
...
function whatDayIsIt(){
  let day = date.getDay()
  let dayName = ["domenica", "lunedì", "martedì", "mercoledì", "giovedì", "venerdì", "sabato"]
...
console.log("Totale: ", risultato.sum);
console.log("Valori: ", risultato.valori);
...
function howManyDays(data){
    ...
    let differenza = (Math.round((Date.parse(dataOggi)- Date.parse(data)) / (24*60*60*1000))*1);
    console.log(differenza)
}
howManyDays("2001-01-30");
...
function isTodayMyBirthday(){
    let dataOggi = new Date();
    let month = dataOggi.getMonth() + 1;
    let day = dataOggi.getDate();
    if(month === 1 && day === 30){
        return true;
    } else {
        return false;
...
function searchByTitle(str){
    return movies.filter(film => film.Title.includes(str));
}

let risultatoFilm = searchByTitle("Avenger");
...
const selectorId = function (){
    let div = document.querySelector("#conainer");
}
...
const linkColorati = function(){
    let link = document.querySelectorAll("a");
    link.forEach(ele => ele.style.backgroundColor = 'red')
}
linkColorati();
...
/* const svuotamentoList = function (){
...
const aggiungereTr = function(){
    let elementiTr = document.querySelectorAll("td");
...
const isItPrime = function (n){
    ...
    if(conta == 1){
        console.log(n + " è numero primo");
    } else {
        console.log(n + " non è numero primo");
    }
}
isItPrime(3);
